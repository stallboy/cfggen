...
- **语言**：中文叙述，代码标识符保留英文。
- **引用源码**：用**相对路径 + 符号名**，不写行号（行号会漂移）。例如：`Context`（见 `ctx/Context.java`）。从本目录出发，源码在 `../src/main/java/configgen/`。
- **代码摘录**：默认只链接不粘贴；确需引用时贴 < 10 行并标注来源文件。
- **图**：用 mermaid 代码块画数据流 / 模块关系。
- **不写逐类清单**：写"职责 / 流程 / 不变量"；必须列时用"关注点 → 主类"小表（稳定），不穷举。要查实际有哪些类，用 Glob（如 `schema/*.java`），别依赖文档里的枚举。
