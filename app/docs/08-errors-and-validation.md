...
`checkErrors(prefix, allowErr)`（见 `../src/main/java/configgen/value/CfgValueErrs.java`）的逻辑：
...
| `-gen` | 类 | 作用 |
|---|---|---|
| `verify` | `ValueVerifyTool`（见 `../src/main/java/configgen/tool/ValueVerifyTool.java`） | `makeValue()` **严格模式**——有值错即抛，本身就是一个**校验闸**；可选 `-gen verify,unreferenced` 报未被引用记录、`-gen verify,entry` 报 entry 记录（entry / enum / root 视为被引用） |
