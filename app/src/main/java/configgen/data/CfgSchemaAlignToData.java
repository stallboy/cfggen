...
public class CfgSchemaAlignToData {
    ...
    private Map<String, FieldSchema> alignFields(TableSchema table, List<DField> header) {
        ...
        for (int idx = 0; idx < size; ) {
            ...
            if (curField != null) {
                int span = Spans.fieldSpan(curField);
                idx += span;
    ...
    private FieldSchema findAndRemove(List<DField> headers, int index, Map<String, FieldSchema> curFields) {
        ...
        if (listField != null
                && listField.type() instanceof FieldType.FList fList && Spans.simpleTypeSpan(fList.item()) == 1
        ...
        if (mapField != null
                ...
                && Spans.simpleTypeSpan(fMap.key()) == 1 && Spans.simpleTypeSpan(fMap.value()) == 1
