...
class TermUpdaterTest {
    ...
    void testEscapeCsv() {
        // 测试空值和空字符串
        assertEquals("", TermUpdater.escapeCsv(null));
        assertEquals("", TermUpdater.escapeCsv(""));

        // 测试普通字符串（不需要转义）
        assertEquals("hello", TermUpdater.escapeCsv("hello"));
        assertEquals("测试", TermUpdater.escapeCsv("测试"));

        // 测试包含逗号的字符串
        assertEquals("\"hello,world\"", TermUpdater.escapeCsv("hello,world"));
        assertEquals("\"测试,逗号\"", TermUpdater.escapeCsv("测试,逗号"));

        // 测试包含双引号的字符串
        assertEquals("\"hello\"\"world\"", TermUpdater.escapeCsv("hello\"world"));
        assertEquals("\"测试\"\"引号\"", TermUpdater.escapeCsv("测试\"引号"));
