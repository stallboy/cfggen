...
- **大文件处理**: 当.cfg文件包含大量表和结构定义时，所有功能应保持流畅，不出现性能问题
  - >5k行文件：所有操作<2秒响应
  - 10k+行文件：不出现卡顿，语法高亮保持实时

- **跨模块引用**: 当引用位于不同模块/目录的表或类型时，正确解析模块名和文件路径
  - 遇到模块名歧义时：显示"模块名不唯一"错误，提示用户使用完整路径[module.]table或重命名目录
...
- **FR-006**: 系统必须处理跨模块引用，通过标准目录结构解析模块名（标准结构：config/[module]/[module].cfg；模块名解析：截取目录名第一个"."之前的部分，再截取"_汉字"或纯汉字之前的部分；遇到模块名歧义时显示"模块名不唯一"错误，提示用户使用完整路径）
- **FR-007**: 系统必须在引用目标不存在时显示明确的"未找到定义"提示
- **FR-008**: 系统必须支持大文件处理，采用增量解析和缓存策略确保性能（>5k行文件操作<2秒，10k+行文件不卡顿）
- **FR-009**: 系统必须对所有语法错误和不完整结构提供清晰的错误提示（红色波浪线实时标注，悬停显示详情，按F7显示问题列表，支持快速导航和批量查看）
...
### Performance Targets (大文件场景)

- **大文件性能**: 当.cfg文件>5k行时，所有语言功能（语法高亮、跳转定义、自动补全）响应时间<2秒
- **语法高亮**: 无论文件大小，始终保持实时响应<50ms
- **大文件支持**: 支持10k+行配置文件不出现性能问题

## Clarifications

### Session 2025-11-09

- **Q: 性能指标是否需要量化？** → **A: 仅针对大文件设指标**（文件>5k行时，所有功能响应时间<2秒，语法高亮保持实时<50ms）
- **Q: 非标准目录结构如何处理？** → **A: 非标准目录就忽略**（如果.cfg文件不遵循标准目录结构[module]/[module].cfg，则跳过不解析）
- **Q: 语法错误提示机制？** → **A: 实时诊断+悬停详情+F7错误列表**（红色波浪线标注，悬停显示详情，按F7显示所有错误）
- **Q: 模块名歧义如何解决？** → **A: 提示错误并让用户手动指定完整路径**（遇到歧义时显示"模块名不唯一"错误，提示用户使用完整路径[module.]table或更改目录名）
