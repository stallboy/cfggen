...
本文档的设计在五大共识上完全对齐行业最佳实践，在三大分歧中做出了有明确理由的选择。最关键的建议是：

1. **保留协程逃生舱**（`ScriptAct`），否则复杂场景会倒逼策划用怪异的树结构绕行
2. **为跨作用域持久效果提供显式语义**（`persist` 标记或作用域提升指引），否则 RAII 会被绕过
3. **增加 "outcomes 为空则 rootAct 完成即终止" 的默认规则**，让简单场景零摩擦