...
// 来自：other/signin.csv
public class CfgSignin {
    private int id;
    private java.util.Map<Integer, Integer> item2countMap;
    private java.util.Map<Integer, Integer> vipitem2vipcountMap;
    private int viplevel;
    private String iconFile;
    private java.util.Map<Integer, config.other.CfgLoot> RefVipitem2vipcountMap;

    private CfgSignin() {
    }

    public static CfgSignin _create(configgen.genjava.ConfigInput input) {
        CfgSignin self = new CfgSignin();
    ...
    public java.util.Map<Integer, config.other.CfgLoot> refVipitem2vipcountMap() {
    ...
    public void _resolveDirect(config.ConfigMgr mgr) {
        if (vipitem2vipcountMap.isEmpty()) {
            ...
            for (java.util.Map.Entry<Integer, Integer> e : vipitem2vipcountMap.entrySet()) {
                config.other.CfgLoot rv = mgr.other_loot_All.get(e.getValue());
    ...
    public static CfgSignin get(int id) {
        config.ConfigMgr mgr = config.ConfigMgr.getMgr();
        return mgr.getOtherSignin(id);
    }

    public static java.util.Collection<CfgSignin> all() {
        config.ConfigMgr mgr = config.ConfigMgr.getMgr();
    ...
    public static class _ConfigLoader implements config.ConfigLoader {
        ...
        public void createAll(config.ConfigMgr mgr, configgen.genjava.ConfigInput input) {
            ...
            for (; c > 0; c--) {
                CfgSignin self = CfgSignin._create(input);
        ...
        public void resolveAll(config.ConfigMgr mgr) {
            for (CfgSignin e : mgr.other_signin_All.values()) {
                e._resolve(mgr);
            }
        }
