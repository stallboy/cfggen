...
public class CfgEquipconfig {
    ...
    private CfgEquipconfig() {
    }

    public static CfgEquipconfig _create(configgen.genjava.ConfigInput input) {
        CfgEquipconfig self = new CfgEquipconfig();
    ...
    public static CfgEquipconfig get(String entry) {
        config.ConfigMgr mgr = config.ConfigMgr.getMgr();
        return mgr.getEquipEquipconfig(entry);
    }

    public static java.util.Collection<CfgEquipconfig> all() {
        config.ConfigMgr mgr = config.ConfigMgr.getMgr();
    ...
    public static class _ConfigLoader implements config.ConfigLoader {
        ...
        public void createAll(config.ConfigMgr mgr, configgen.genjava.ConfigInput input) {
            ...
            for (; c > 0; c--) {
                CfgEquipconfig self = CfgEquipconfig._create(input);
