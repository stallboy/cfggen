...
> **本文档保留**：（无）——批次3、批次4 已全部落地，下文转为决策记录。
> **2026-07-10 落地**：批次3（P1 内联 `getId`、P2 `registerPrefKeySet`）、批次4（`recordRefEntity→recordRefUtils`）。lint + build + 203 单测全过。
> **2026-07-10 修订**：批次3 原方案（迁 FixedPage 工厂到 domain）经代码核实会制造 `domain→store` 反向依赖，已否决，改最小治理（只动 `getId`），详见第二节。

---

## 一、依赖问题（P1/P2 已于批次3 解决）
...
## 二、批次3 · 反向/循环依赖治理（✅ 已落地 2026-07-10）

> **落地摘要**：P1—删 store.ts 的 `getId` import，`makeFixedPage` 内联 `` `${curTableId}_${curId}` ``；P2—storage.ts 删 store import、新增 `registerPrefKeySet`，store.ts 初始化时注册键集。`store→recordRefEntity` 反向边、`store↔storage` 循环均已消除。下文方案/否决理由保留作决策记录。
...
## 三、批次4 · 命名（✅ 已落地 2026-07-10）

`recordRefEntity.ts → recordRefUtils.ts`（含 `.test.ts`，`git mv` 保护 blame）。

> **落地**：P1 已先消除 store.ts 对 recordRefEntity 的引用，故实改 **5 个 importer**（4 生产 + 1 test）：Record.tsx、recordEditEntityCreator.ts、RecordRef.tsx、recordEntityCreator.ts、recordRefUtils.test.ts；另同步 CLAUDE.md:48 测试覆盖列表。lint + build + 203 单测全过。

- 改名理由：该文件是一组函数（`getId` / `getLabel` / `createRefs` / `createRefEntities`...），无类、不定义 Entity 类型，`Entity` 后缀与 `flow/entityModel` 的 `Entity` 核心类型混淆。
- 仅改名，未触及「recordRefEntity 被跨层引用却置于 `routes/record/`」的位置错位——P1 已用内联消除该跨层引用，移位 ROI 进一步下降，不做。
