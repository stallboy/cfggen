...
import {makeNodeShow, makeReadOnly, makeEditable, makeCard, editWith} from '@/test/fixtures'
...
describe('getNodeWidth', () => {
    ...
    it('editable + 覆盖 editNodeWidth → 覆盖值', () => {
        expect(getNodeWidth(makeEditable({id: '1', label: 'x', edit: editWith([])}), makeNodeShow({editNodeWidth: 500}))).toBe(500)
    })
    ...
    it('非 edit(readonly) + 覆盖 nodeWidth → 覆盖值', () => {
        expect(getNodeWidth(makeReadOnly({id: '1', label: 'x', fields: []}), makeNodeShow({nodeWidth: 320}))).toBe(320)
    })

    it('card 实体（非 edit）走 read 宽度', () => {
        expect(getNodeWidth(makeCard({id: '1', label: 'x', brief: {value: ''}}), makeNodeShow({nodeWidth: 199}))).toBe(199)
    })
})
