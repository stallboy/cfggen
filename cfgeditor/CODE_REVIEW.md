...
| 第6批 | O13 | ✅ | `b1f7e46b` | 删 embedding 死类型（O11 经评估保留原 swap，见未做项） |
...
- **A3（内嵌判定条件 checker/extractor 复制）**：未做。embeddingFieldExtractor 逐行复制 checker 的 5 条内嵌条件，且 recordEditEntityCreator 两处独立计算「是否内嵌」。消除漂移需跨 3 文件重构 + 回归测试，待专项。
...
- **O11（onMoveItemInArray）**：经评估**保留原始 swap 实现**（已从 splice move 回退）。所有调用方均传相邻索引（上/下移按钮），swap 与 splice move 对相邻索引行为完全一致；命名虽含 move 但无实际顺序错乱场景，保留 swap 以免引入不必要的行为差异。
