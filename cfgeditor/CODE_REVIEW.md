...
| # | 位置 | 问题 | 修复 |
|---|---|---|---|
| **C5** | `routes/table/schemaUtil.tsx:262-263` | `else if (t.startsWith('list<') \|\| t.startsWith('map<')) { res[n] = []; }` —— `map<K,V>` 字段默认值给成空数组 `[]`，应为 `{}` | 拆开两分支，`map<` 给 `{}` |
...
### A1. 模块级可变单例 `editState` 与分屏布局本质冲突

- **位置**：`routes/record/editingObject.ts:33-45`（单例）、`Record.tsx:112`（注释自认「非纯函数 escape hatch」）、`Record.tsx:122-124`（把 `edit` 写进全局 store）
- **问题**：整个编辑会话状态（editingObject/original/copied/回调）挂在进程级全局变量上。项目**已实现分屏布局**（cfgeditor CLAUDE.md「分割布局系统」），一旦同时打开两个 Record 编辑面板，后打开的 `startEditingObject` 会覆盖前一个的 `table`/`id`/`editingObject`。
- **失败场景**：左面板编辑 TableA·记录X，右面板打开 TableB·记录Y → `editState` 被右面板覆盖 → 左面板用户继续打字 → 实际改的是右面板的 `editingObject`，左面板提交时把 Y 的数据提交到 X。
- **修复建议**：把 `editState` 收敛为 `Record.tsx` 内的 `useRef`/局部状态（`:112` 注释已提到 useRef 可替代），editingObject/original/copied/回调作为 props 或 context 下发；`setIsEditMode` 也应避免由路由组件写全局。
...
**第 3 批（持久化可靠性 C13/C14/C15 + A1 editState 单例）**
