...
| **C13** ❌误判（已撤销 `970b7681`，详见第九章进度表） | `CfgEditorApp.tsx:62-80` | React Query 的 `select` 回调内调用 `clearLayoutCache()`（`queryClient.removeQueries`）——`select` 在 RQ 内部会被反复调用（重渲染、相等性比较），导致已算好的 `['layout']` 缓存被非预期清空、布局反复重算 | 副作用移到 `useEffect`：`useEffect(() => { if (schema) clearLayoutCache(); }, [schema])`；`select` 保持纯转换 |
...
- **O7-antd 🔧 antd 深路径导入（`antd lint` 权威检出）** ✅ 已修（`4ee61d6b`）：以下 6 处从 antd 内部路径 `antd/es/*` 或 `antd/lib/*` 导入，违反 antd 最佳实践、破坏 tree-shaking 一致性，且 `es`(ESM)/`lib`(CJS) 混用、跨版本易碎。`antd lint` 把其中 3 处 default 导入直接判为 `performance: error`：
...
| 批 | 项 | 状态 | Commit | 说明 |
|---|---|---|---|---|
| 第3批 | C14/C15 | ✅ | `c2700e3b` | setPref debounce(300ms)+写入串行化；关窗 async+preventDefault+destroy |
| 第3批 | C13 | ❌误判已撤销 | `c2700e3b`→`970b7681` | **误判**：select 的 clearLayoutCache 是原作者有意（驱动 layout query 失效重取、带动 RecordWithResult 重渲染）。移到 useEffect 后 SPA navigate（右键切浏览/编辑、重选已访问 id）视图不刷新。已撤销恢复（`970b7681`），Chrome DevTools MCP 实机验证 |
| 其他 | setState-in-render | ✅ | `897e15a9` | resetEditingObject 经 startEditingObject 在 useMemo(render 期)调 notifyEditingState→setEditingState 触发 React "Cannot update component while rendering"。notify 移到 Record useEffect（诊断 C13 时发现，原 review 未列出）|
...
| Optional | O7-antd | ✅ | `4ee61d6b` | 6 处 antd 深路径导入改包根具名导入；`antd lint ./src` 归零（useForm 根未导出，改 Form.useForm） |
| Optional | O1 | ✅⚠️待实机 | `7860477a` | elk 布局迁到 Web Worker（`elk-worker.min.js?url` + 模块级 `new ELK({workerUrl})`，算法不阻塞主线程；build 生成独立 worker asset） |
...
- **O1（elkjs 主线程→Web Worker）**：✅ 已做（见进度表 `7860477a`），仅运行期待实机验证（worker 加载/布局正确/主线程不冻结）。
