...
> ⚠️ 本文**不重复**已有的详细描述:
> - 编辑态内部(useSyncExternalStore、脏标记、就地变异)→ [`04-state-management.md`](./04-state-management.md) §6、§7(§7 本就是"一次编辑的全链路" prose 版)
> - undo 快照栈 / coalescing / 提交边界 → [`06-undo-redo.md`](./06-undo-redo.md) §2.5、§3
> - React Query 缓存 / mutation / 失效 → [`05-url-api-reactquery.md`](./05-url-api-reactquery.md) §5、§6
>
...
这条**读路径**本文不展开(它和数据缓存、queryKey 设计深度绑定)——见 [`05-url-api-reactquery.md`](./05-url-api-reactquery.md) §5.1。下面只讲**写路径**。
...
- 细节:[`状态管理`](./04-state-management.md) §6.1 / [`undo-redo`](./06-undo-redo.md) §2.2。
...
- 细节:[`undo-redo`](./06-undo-redo.md) §3.5。
...
- 细节:[`undo-redo`](./06-undo-redo.md) §3.9。
...
- 细节:[`状态管理`](./04-state-management.md) §6.5 / [`undo-redo`](./06-undo-redo.md) §3.6 / [`url-api-reactquery`](./05-url-api-reactquery.md) §6.1。
