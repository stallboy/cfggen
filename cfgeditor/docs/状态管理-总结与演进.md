...
> 文档读码过程中发现的具体可改点，按确定性排序。均为**建议**，未改动代码。

### 1. `if (value)` 误杀 `0`（确凿 bug 隐患）

`store.ts` 有 11 个 `number | null` setter 用 `if (value)` 判空（`setMaxImpl` / `setMaxNode` / `setRefOutDepth` / `setRecordMaxNode` / `setRefIdsMaxNode` / `setSearchMax` / `setImageSizeScale` 等）。`value` 为 `0` 时 `if (value)` 为假 → **直接跳过，既不改 resso 也不持久化**。`null` 才是"无操作"哨兵，`0` 是合法值（如 `searchMax = 0` 表示不返回结果、`maxNode = 0` 表示不展开）。

**改法**：`if (value !== null)`（与 `null` 哨兵语义对齐）。boolean setter 用 `setPref('x', checked ? 'true' : 'false')` 无此问题。

### 2. 关窗只 flush 个人偏好，共享偏好可能丢（数据完整性）

`main.tsx` 的 `onCloseRequested` 只 `await saveSelfPrefAsync()`（写 `cfgeditorSelf.yml`），**不 flush `cfgeditor.yml`** 的 pending 防抖写。若用户在关窗前 300ms 内改过共享配置（`maxImpl` / `nodeShow` / `themeConfig` / `refOutDepth`…），那次防抖定时器还没 fire，窗口就被 `destroy()` → 变更丢失。

**改法**：新增 `saveSharedPrefAsync()`（或统一的 `flushAllPrefsAsync()`），关窗时 `await` 个人 + 共享两份 pending 写。

### 3. `getIsEdited` 每键 O(n) 全量深比较（性能）

每次值类编辑 `updateFormValues → notifyEditingState → getIsEdited → isDeeplyEqual(editingObject, originalEditingObject)` 对整棵编辑对象做深比较。大 record（嵌套深、数组多）连续键入时，每键一次 O(n)。

**改法**：维护 `private dirty = false` 脏标记——任何 mutation 置 `true`（O(1)）；`undo/redo` 后重算一次（用户触发，低频）；`resetBaselines` / `onCommitSuccess` 置 `false`。`getIsEdited()` 直接返回 `dirty`。热路径从 O(n) 降为 O(1)，同时顺带收敛 §8.2 的"三处判定"可读性负担。注意 undo 可能恰好回到 baseline（变 clean），需在 `undo/redo` 路径重算而非简单翻转。

### 4. `toInt` / `toFloat` 静默吞 NaN 为 `0`（数据完整性 / UX）

`editingSession.ts` 的 `toInt`/`toFloat`：`parseInt('abc') → NaN → 返回 0`。用户在 int/long/float 字段输入非法字符串，会被静默写成 `0` 提交，无任何反馈。注释自己也标注了这一点。

**改法（择一）**：非法输入时保留原值（不写回，让 antd Form 校验态提示）；或返回 `null`/原字符串交由 Form 规则拦截。取决于产品想要"宽松容错"还是"严格校验"——现状是无声的中间态，最差。

### 5. 文档/可维护性：行号引用 → 符号引用（已在本次重写中处理）

原正文大量 `file:line` 指针随重构漂移（多处已失效）。本次已改为「文件 + 导出符号」定位。后续新增文档沿用此约定。
