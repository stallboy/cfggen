...
判定、数据与状态机（读侧 `classifyListField` + 写侧归一化 `normalizeOnX`）全在 [embedding.ts](../src/domain/embedding.ts)——embed 概念的唯一家。session 在 undo 括号内 apply 决策、creator 注入 itemType（SStruct|SInterface），两者都不拥有状态机本身（判定 + 决策都在 domain）。
...
读取（`getEmbedState(obj, fieldName)`），**先定类、再解读**——`getEmbedState` 与 `classifyListField` 现统一驻 [embedding.ts](../src/domain/embedding.ts)。list 字段的类判定收口在 `classifyListField`（createEntity 子节点循环与 createListOrMapEditField 两条读路径共用，防止漂移）：
...
类边界只可能被三类操作跨越。归一化**决策**（`normalizeOnAdd` / `normalizeOnDelete` / `normalizeOnImplSwitch`，纯函数返回 `KeyOp = 'writeFalse'|'delete'|'noop'）驻 [embedding.ts](../src/domain/embedding.ts)；session 在 undo 括号内 apply 决策（`applyKeyOp`）——**与用户操作同一步 undo**（[editingSession.ts](../src/services/editingSession.ts)）。判定与决策都在 domain：`normalizeOnX` 自调 `canBeEmbeddedCheck`，creator 传 `itemType`（SStruct|SInterface）；session 只 apply、不碰判定。

**① `addArrayItem` / `addArrayItemAtIndex(defaultItem, ..., itemType)`**：

- **0→1 且新元素可内嵌**（判定由 `normalizeOnAdd` 自调 `canBeEmbeddedCheck`，`itemType` 由调用方传入）：写 `$embed_xxx=false`——新元素默认展开成节点、立即可编辑（原 `markNewItemExpanded` 语义的新家：从往子对象写 `$fold=false` 改为往父对象写 `$embed_xxx=false`）。
- **其余（≥2，或单元素不可内嵌）**：删键——同时覆盖「折叠中加元素自动展开」（原语义）与「1→2 后 false 键变残留」两种情形。

**② `deleteArrayItem(index, ..., anchorId, itemType)`**：
...
幸存元素（`arr[0]`）的可内嵌性由 `normalizeOnDelete` 自调 `canBeEmbeddedCheck` 判定（`itemType` 由 creator 传入，不再预算）。

**③ `updateInterfaceValue(newObj, ..., itemType)`**（impl 切换）：
...
可内嵌性由 `normalizeOnImplSwitch` 自调 `canBeEmbeddedCheck` 判定（`sInterface` 由 `interfaceOnChangeImpl` 传入）。
