...
| 讨论 | FlowGraph 菜单反向注入、fillHandles 变异 entity | 低（多为认知） | — | §5-A1/A2、§6 |
...
#### A2. `fillHandles` 变异 entity，与"entity 不可变"叙事矛盾

**诊断**：`entityToNodeAndEdge.ts:27-58` 的 `fillHandles` 直接 `entity.handleOut = true` / `field.handleIn = true` —— 在 **mutate** entity。但 `colors.ts`/`FlowGraph.tsx` 大段注释强调"entity 保持纯 domain、不可变 memo-safe"。

**真相**：`handleIn/handleOut` 是 entity 上**有意保留的可变标志位**（由路由层 `fillHandles` 在 entityMap 构造后盖章，供 `convertNodeAndEdges` 与 `FlowNode`/`EntityProperties`/`EntityForm` 读）。这是历史设计，工作良好。

**建议（认知统一，非必须改）**：二选一——
- **承认它**：在 `entityModel.ts` 的 `Entity` 上给 `handleIn/handleOut` 加注释，明确"这两个是 entityMap 构造期由 `fillHandles` 盖章的可变呈现标志，是 `纯 domain` 的例外"。让叙事自洽。
- **搬家**：像 `nodeShow/notes` 那样，把 handle 信息也算进 `node.data`（转换时由 `sourceEdges` 派生），entity 彻底不可变。改动面较大（`FlowNode`/`EntityProperties`/`EntityForm` 读 handle 的地方都要改），收益是纯度。按 `docs/设计标准优先于便利度` 的判据，**搬家是"正解"**，但优先级低于 B/C 拆分。
...
1. **(低风险) 对齐 note 估算与渲染**：要么渲染侧也按估算的多行展开（`rows` 随估算值），要么估算侧改成与 `rows={1}` 一致的单行 + overflow。选后者更省空间。改后跑 `calcWidthHeight.test.ts` 确认。
...
#### F1. `FlowNode` 的 `let fields/edit/brief/note/assets = undefined` 改判别联合

**诊断**：`FlowNode.tsx:57-74` 是命令式"类型守卫后赋值"：
```ts
...
```
这丢掉了判别联合的类型收窄收益，下游 `fields && <EntityProperties>`、`edit && <EntityForm>`、`brief && <EntityCard>` 全靠 truthy 而非类型。

**做法**：抽一个 `pickEntityView(entity)` 返回判别联合，FlowNode 用 switch 渲染：
```ts
type EntityView =
    | {kind: 'readonly'; fields: DisplayField[]; note?: string; assets?: ResInfo[]}
    | {kind: 'editable'; edit: EntityEdit; note?: string}
    | {kind: 'card'; brief: EntityBrief; note?: string; assets?: ResInfo[]};
```
这与 C1/C2 的拆分天然契合（每个 kind 对应一个子渲染分支）。`colors.ts` 的 `getEntityValueString` 也在做同类三分支，可共用这个 view 模型。
...
| **P1** | §5-C：FlowNode 拆 NodeNote/NodeTitle/NodeToolbar + F1 判别联合 | 中 | 高（可读/可改） | 各视图手测 note/fold/moveUp/delete 行为不变 + Profile 看 per-node 渲染耗时未升 |
...
| 讨论 | §5-A2 fillHandles 搬 node.data、§5-A1 契约文档化 | — | 认知 | — |
...
src/flow/
  ...
  FlowNode.tsx                   瘦身：外壳 + pickEntityView 分发（~80 行）
