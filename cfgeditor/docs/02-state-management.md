...
>
> 【承前】01 的 React Query 是其中一种（远端态）。　【启后】五种里的 `EditingSession` 是编辑能力地基 → [03](03-editing-session-undo.md)。
...
| **resso store** | 业务 UI 态（29 个 key）| [`store.ts`](../src/store/store.ts) + [`resso.ts`](../src/store/resso.ts) | render 期同步读、事件期写 |
...
**为什么不只用一种**：远端数据要缓存 + 失效（RQ）；UI 态要同步读 + 细粒度订阅（resso）；偏好要跨会话（storage）；编辑态要可变 + undo（EditingSession）。各取所长——混用是 bug 源，但强行统一更糟。
...
const resso = <Obj extends Record<string, unknown>>(obj: Obj): Store<Obj> => {
  // 每个 key 独立 { subscribe, getSnapshot, triggerUpdate }
  Object.keys(obj).forEach((key) => {
    const setters = new Set<VoidFn>();
    state[key] = {
      subscribe: (setter) => { setters.add(setter); return () => setters.delete(setter); },
      getSnapshot: () => obj[key],
      triggerUpdate: () => setters.forEach((s) => s()),
    };
  });
  return new Proxy(/* ... */, {
    get: (_t, key) => {
    ...
    set: (_t, key, val) => { setKey(key, val); return true; },
  });
};
```

核心：`get` 命中某 key 时调 `useSyncExternalStore` 订阅**该 key**，`set` 时对比 `oldVal !== newVal` 后只触发该 key 的订阅者。
...
```ts
const store = resso<StoreState>(storeState);   // store.ts:249
export function useMyStore() { return store; }
```
...
`StoreState`（store.ts:32）29 个 key，按用途分组：
...
| **编辑模式开关**（持久化、URL 派生）| `isEditMode` | URL `/edit` 段派生（Record.tsx:149 `setIsEditMode(edit)`），写个人 pref |
...
```ts
// store.ts setMaxImpl 注释（:268）：本 setter 不清 layout 缓存——
// topologyKeys 已进 layout queryKey，改值自然失效，store 重新变纯状态容器。
```

store setter 重新变**纯状态容器**（`setMaxImpl` / `setNodeShow` 注释明说），把缓存失效让位给 queryKey。
...
`editingCurTable` / `editingCurId` / `editingIsEdited` 三个 key 不持久化（`notSaveKeySet`），**只由 EditingSession 通过 `onEditingStateChange` 回调写**（Record.tsx:91 调 `setEditingState`）——HeaderBar 订阅它们显示 unsaved 脏点。store 不直接管编辑态，只镜像显示。
...
```ts
const prefSelfKeySet = new Set(['curPage','curTableId','curId','query','isEditMode',
                                'imageSizeScale','dragPanel','aiConf']);  // aiConf 含 apiKey，写个人文件
const notSaveKeySet  = new Set(['history','resMap','resourceDir',
                                'editingCurTable','editingCurId','editingIsEdited']);  // 编辑镜像不持久化
// prefKeySet = Object.keys(storeState) − prefSelfKeySet − notSaveKeySet  → 共享文件
```

> **注**：`prefSelfKeySet` 里的 `curPage` / `curTableId` / `curId` **不在 `StoreState` 类型里**——它们是路由派生的 localStorage key（`navTo` 直接 `setPref` 写，见 §一 路由派生行）。故 `StoreState 29 key ∩ prefSelfKeySet` 实际只有 5 个（`query` / `isEditMode` / `imageSizeScale` / `dragPanel` / `aiConf`）。

**依赖方向**：键集由 store 在模块加载时 `registerPrefKeySet` 注册给 storage（store.ts:181），**消除 storage→store 反向依赖**——storage 只持有两个 `Set<string>`，不知道 store 的类型。
...
sequenceDiagram
    ...
    Note over A,F: 第一段：setting query（AppLoader.tsx:9，staleTime:∞ + retry:0）
...
为什么分两段：先让 localStorage 就绪（第一段），再从它同步灌 store（第二段）——避开「store 初始化读到空 localStorage」的竞态。`alreadyRead` 守卫保证各只跑一次（`readPrefAsyncOnce` storage.ts:67 + `readStoreStateOnce` store.ts:185 两份分驻两文件）。

**setting query 配置**（AppLoader.tsx:12-13）：`staleTime: Infinity`（只跑一次，永不 stale 防重读 yml 覆盖内存态）+ `retry: 0`（yml 读失败不无限重试）。**web 模式**（非 Tauri）`readPrefAsyncOnce` 早 return（`if (!isTauri()) return`，不 clear 不读 yml），只走 localStorage。

`readStoreStateOnce`（store.ts:185）按类型分发：复杂对象（`nodeShow` / `aiConf` / `pageConf` / `tauriConf` / `themeConfig`）用 `getPrefJson` + Convert parser；基本类型按 `typeof` 走 `getPrefBool` / `getPrefInt` / `getPrefStr`。
...
```ts
// ① debounce 300ms：合并短时间多次写（navTo 一次触发 3 次 setPref：curPage/curTableId/curId）
function scheduleWrite(fn, keySet) {
    const timer = setTimeout(() => writeFileSerialized(fn, keySet), WRITE_DEBOUNCE_MS);
    writeTimers.set(fn, timer);
}
// ② 串行化：同文件并发写会损坏/丢字段，挂在共享 Promise 链上保证一次只一个 writeTextFile
let writeChain: Promise<void> = Promise.resolve();
function writeFileSerialized(fn, keySet) {
    writeChain = writeChain.then(() => saveKeySetPrefAsync(keySet, fn)).catch(log);
    return writeChain;
}
// ③ 关窗强制 flush（绕过 debounce）：main.tsx onCloseRequested preventDefault → flushAllPrefsAsync → destroy
async function flushAllPrefsAsync() {
    await Promise.all([saveSelfPrefAsync(), saveSharedPrefAsync()]);  // 两次共用 writeChain 串行
}
```

为什么三层都不可少：debounce 省 IO，但 pending 定时器在关窗 `destroy` 前没 fire 就丢——所以关窗要 `preventDefault` + bypass debounce 强制串行落盘（main.tsx:65）。串行化防同文件并发写损坏。
...
**每条 record 编辑态一个 `EditingSession` 实例**（editingSession.ts:40），用 `useSyncExternalStore` 接入 React，承担值类 / 结构类编辑的就地变异、undo / redo、脏标记、视口语义。模块级 `currentEditingSession` 指针让 Splitter 兄弟（Chat / AddJson）跨路由寻址当前会话。
