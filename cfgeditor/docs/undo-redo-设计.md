...
```ts
type Snapshot = { data: JSONObject; undoFitView: EFitView; anchorId?: string };   // 见 undo-redo-视口稳定方案.md：快照带 undo 视口语义（结构→KeepStable+锚点；整体替换→FitFull；值类→NoChange）
...
undo() {
    ...
    const {target, undoFitView, anchorId} = this.undoStore.popUndo();  // ② 被撤销操作快照的语义 + 要恢复成的 target
    this.applyUndoPoint(target.data);             // ③ editingObject = structuredClone(target.data)
    // ④ 按被撤销操作的语义驱动视口：结构→KeepStable+锚点（焦点屏幕不动）；整体替换→FitFull；值类→NoChange（不动）
...
**undo 视口按被撤销操作的语义**：结构 undo 用 `KeepStable`——以「被撤销操作的焦点节点」为锚点（delete 取父），relayout 后让其屏幕坐标不动（anchorOld 由 flow 层从「上一帧布局」读，不用快照里过时的坐标）；值类 undo 布局不变用 `NoChange`；整体替换 undo 用 `FitFull`。详见 [undo-redo-视口稳定方案](./undo-redo-视口稳定方案.md)。
...
- **怎么做**（快照栈）：每次编辑后存 `editingObject` 全量深拷（快照带 undo 视口语义 `undoFitView`/`anchorId`）；undo = apply 上一份快照 + `bumpStructure`（按快照语义：结构→`KeepStable`+锚点 / 值类→`NoChange` / 整体替换→`FitFull`）；redo 对称。`UndoStore` 纯数据类，`undo/redo` 是 session 方法。
