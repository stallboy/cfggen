...
枚举用于在 schema 文件中定义编译时常量。支持两种语法格式：
...
```cfg
// 在.cfg中配置table
table ArgCaptureMode[name] (enum='name') {
    name:str;
    comment:str;
}
...
comment 字段默认为 `str`——枚举注释是策划/开发向备注，不进入多语言体系。如果确实需要国际化（如展示给玩家的枚举描述），加 `commentText` 标记：
...
// 此时 comment 字段为 text 类型
```

### 有赋值形式

语法格式：`enum 名称 { 值 = 数字; 值 = 数字; }`
...
```cfg
table StatClampMode[name] (enum='name') {
    name:str;
    id:int;
    comment:str;
}
...
// 也支持list<ArgCaptureMode>和map<str, ArgCaptureMode>等容器类型
```

| 特性 | 无赋值 Enum | 有赋值 Enum | Table Enum |
|------|-------------|-------------|------------|
| 语法 | `enum A { X; Y; }` | `enum A { X = 0; Y = 1; }` | 配置在 Excel 中 |
| 主键 | name | name | 可定制 |
| 数据位置 | .cfg 文件 | .cfg 文件 | Excel 中配置 |
| 枚举值 | 无数字编号 | 有数字编号 | 策划动态扩展 |
...
- **enumRef**（可选）：指向枚举表，该表包含接口所有实现类的信息
  - 作用：让策划在Excel中知道有哪些可填，无需查看CFG文件
  - 要求：枚举表必须包含实现类名称，可包含ID和其他信息

- **defaultImpl**（可选）：指定默认实现类，若不设置默认是interface里第一个struct。
  - 作用：当策划不填写接口字段时，自动使用默认实现，避免数据不完整
...
| `seq` | 声明字段值为连续序列（从0开始） | int | `tagId:int (seq)` |
...
| 构造 | 语法 | 用途 |
|------|------|------|
| 枚举 | `enum 名称 { 值; }` 或 `enum 名称 { 值 = 数字; }` | 定义编译时常量枚举 |
...
| `commentText` | enum 声明 | comment 字段用 text 类型（默认 str） |
| `nullable` | 字段 | 允许为空 |
| `mustFill` | 字段 | 必须填写 |
| `seq` | 字段 (int) | 声明值为连续序列（0,1,2,...） |
| `fix=N` | 映射... | ... |
| `block=N` | 映射... | ... |
| `pack` | 映射... | ... |
| `sep='分隔符'` | 映射... | ... |

> 上表属性均为保留名，只能出现在"位置"列标注的声明上；放错位置（如 `json` 写到 struct 上）、纯 tag 带值（`json='y'`）或必带值裸写（`(sep)`、`(enum)`）都会在解析时报错，不再静默忽略。
