...
> **注意**：本节介绍的是通过 Table 的 `enum` 属性定义的枚举。
> 如果需要在 schema 文件中直接定义编译时常量枚举（只有 name 和 comment），
> 请参考 [结构定义 - 枚举](../schema/#枚举enum)。

为了避免在代码中出现大量的"魔数"（magic numbers），配表系统提供了枚举和入口机制，
直接在配置表中定义程序使用的英文名称，这些名称会被生成到代码中。
