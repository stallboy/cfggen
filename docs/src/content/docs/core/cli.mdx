...
Usage: java -jar cfggen.jar [options] [-tool toolName[,param=value...]] -datadir <dir> [-gen genName[,param=value...]]
   ...
   or: java -jar cfggen.jar [-h] (print help)
   or: java -jar cfggen.jar -gui (launch GUI)
...
-----generators
    ...
    -gen java        生成java代码
        ...
        noSealed             ,默认为false
        snakeEnumName        enum/entry常量字段名用SCREAMING_SNAKE_CASE（如ResetDuration->RESET_DURATION），默认false为大写直连
