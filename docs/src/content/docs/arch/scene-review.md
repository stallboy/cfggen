...
> 本文是设计评审（决策备忘 + 行业对照），不是评审基准。权威定义以 [`scene-design.md`](./scene-design.md) 为准。文中业界信息以公开资料为准，不逐条核实。
...
这是工业界长期的趋势，从“程序员写脚本”走向“策划填表/拖图”。
...
“Boss 死了但无敌 Buff 还在”、“过场结束但镜头锁死” 是常见的 P0 Bug 类别。
...
但必须承认：极度复杂的时序逻辑（如 FFXIV 绝本的机制编排，多个定时器交叉、条件判断层层嵌套）在纯声明式树中会变成“节点爆炸”。这是树为骨架路线的固有边界——超出表达力时，要么拆分场景，要么在受控范围内引入协程脚本（必须实现 `onCleanUp`）。spec 当前未提供脚本逃生舱节点。
...
**取舍代价**：RAII 作用域绑定的固有约束是“效果难以跨作用域存活”。需要跨 Phase 持续的效果，要么把 `WithStatus` 提到 `StateMachine` 外层（提升作用域），要么交给 GAS 管理（用普通 ApplyStatus，由 GAS 而非场景负责其生命周期）——这是“谁挂载谁清理”的代价：场景只清理自己施加的副作用。spec 当前没有跨作用域 persist 语义，靠手动提升作用域解决。快照回滚更适合作为 debug 工具或兜底安全网，而非默认清理机制（内存、性能、一致性成本高）。
...
代价是简单线性演出（一段过场动画）也得显式配 outcome，略显仪式感。spec 用显式 outcomes 管理场景生死，没有“为空则默认成功”的隐式规则；若日后想为简单场景减负，可以考虑这条隐式规则，但属于未来可选优化。
...
本文档的设计在五个共识上对齐业界常见做法，在三个取舍中做出了有明确理由的选择。