---
title: Ability 系统 review
sidebar:
  order: 4
---

> 本文是设计评审（决策备忘 + 行业对照），不是评审基准。权威定义以 [`ability-design.md`](./ability-design.md) 为准；本文记录"为什么这样设计、不那样设计"，以及业界怎么对照。文中业界信息以公开资料为准，不逐条核实。

## 5 个常见思维模式

### 1. 数据驱动优于硬编码

战斗逻辑应由数据表/配置驱动，而非散落在代码各处的 if-else。这使得策划能独立迭代，减少程序员参与每个技能的实现。GAS、《Dota2》的 KV 系统、《英雄联盟》后期重构都走向了这个方向。

### 2. 标签作为通用语言

用层级化标签（而非枚举/硬编码 ID）作为系统间的通信协议，是业界常见做法。标签的松耦合特性允许新机制在不修改已有代码的前提下接入系统。从 GAS 的 GameplayTag 到 Unity 的 Tag 系统，都有采用。

### 3. Effect/Modifier 的分离

"瞬时效果"与"持续修改器"的正交分离是常见的抽象方式。将一次性动作（扣血、位移）与持续状态（加攻 buff、减速 debuff）在架构上隔开，避免了生命周期管理的混乱。这对应文档中 Effect（无状态原子指令）与 Status+Behavior（有生命周期的状态容器）的分离。

### 4. 事件管线化的结算流程

伤害/治疗等核心流程应该是可注入的管线（Pipeline），而非单一函数调用。Pre/Post 阶段的拦截点允许任意 buff 以解耦方式篡改或响应结算过程。GAS 的 GameplayEffectExecutionCalculation、《暗黑3》的伤害管线都采用了这种管线化结算。

### 5. 逻辑与表现的严格隔离

技能逻辑层不应直接引用任何视觉/音频资源。通过间接标识符（Cue）让客户端自行映射资源，这在网络同步、跨平台、性能分级等场景下是刚需。少见有人主张逻辑和表现耦合。

---

## 3 个关键取舍

### 取舍 1：声明式配置 vs. 脚本式灵活性
...
**对本文档的质疑：**
> "你在文档中用**加粗红色警告**写了'策划配置红线：带有 Payload 的节点只能在带有事件上下文的 Effect 执行链中使用'。但你的类型系统**无法表达**这个约束——`Effect` 接口在结构上不区分'有 Payload 的 Effect'和'无 Payload 的 Effect'。`OnApply` 的 `effect: Effect` 和 `Trigger` 的 `effect: Effect` 类型签名完全相同。你把一个**结构性安全问题**降格为**文档中的口头约定**。"
>
> "`ArgCapture` 的存在本身就是对类型系统不足的补丁——如果系统能在类型层面区分'有 Payload 上下文'和'无 Payload 上下文'，`ArgCapture` 的一半用例就不需要存在。"
>
...
本文档选择了正确的起点，但需要承认它的天花板。

**声明式是对的，但质疑也是真实的。** 关键在于认清这不是二选一，而是分层问题。

本文档实际上已经在构建一门领域特定语言（DSL）。`Conditional`、`Repeat`、`Sequence`、`WithLocalVar` 就是控制流原语。但它与 Lua 之间有一个根本性区别：
...
2. 当发现策划需要在节点树中"模拟编程"时，不要给他们编程能力——给他们一个新的、封装了该模式的专用节点。
...
**已知局限**：`AllocationLayer` 当前的 `conversionRate: float` 是个标量，表达"只吸收魔法伤害的护盾""按比例分摊到护盾和血量"这类语义会捉襟见肘。这是统一模型在异质化规则交互上的固有代价——真需要这类护盾时，要么扩展该字段，要么承认这里更适合专用组件。当前 spec 未采纳扩展，留作按需评估。

---

### 取舍 3：编译期安全 vs. 运行时灵活

**已知局限**：质疑者说得对——`OnApply` 的 `effect: Effect` 和 `Trigger` 的 `effect: Effect` 类型签名相同，但运行时语义根本不同（前者无 Payload，后者有）。

而且 Payload 依赖不在 Effect 级别，而在 Effect 内部的**叶节点级别**：
...
要做完整的类型安全，需对整棵树做类型推导，这实质上是构建一个类型检查器/编译器，成本过高。所以 spec 选择了运行时容错（空值检查 + 日志 + 安全回退），把 Payload 作用域作为配置约定（红线）而非类型约束。这是"运行时灵活"一端的明确取舍：换来了配置简洁，代价是这类错误要到运行时才暴露。如果未来 Payload 误用频繁，再考虑在工具链里加一道加载期扫描——这是工具链的事，不需要改动配置表结构。



## 著名游戏在三大取舍上的实际抉择

> 业界信息以公开资料为准，不逐条核实；用于对照取舍方向，不作权威引用。

| 游戏 | 取舍1：配置 vs. 脚本 | 取舍2：统一 vs. 专用 | 取舍3：静态安全 vs. 运行时灵活 |
|------|---------------------|---------------------|------------------------------|
...
Statescript 在类型安全上做到了游戏行业的较高标准：
- 所有变量有明确类型声明
- 节点间的连线有编译期类型检查
- 状态机的转换条件在编译期验证可达性
- 网络同步属性有专门的标注和验证

这使得守望先锋的技能 Bug 大多是**逻辑错误**（数值调错、时序不对），而较少是**类型错误**（空指针、类型转换失败）。
...
**关键洞察**：暗黑3证明了"编辑器约束 + 加载验证"的双层策略是较具性价比的——它不需要在配置 Schema 层面做复杂的类型系统设计，但仍然能在策划犯错的第一时间给出反馈。
...
本文档当前处于中间偏右的位置——有外键引用验证，但 Payload 作用域等关键约束只停留在文档层面的"红线警告"。这是一个明确的取舍点：如果 Payload 误用成为高频问题，值得向暗黑3的位置移动（在工具链加一道加载期扫描）；否则维持当前运行时容错即可。
...
本系统的取向：**声明式节点树**表达（接近暗黑3 PowerScript）、**默认统一**的模型、**外键引用 + 配置约定**的安全策略。脚本能力不作为推荐路径——当节点树开始"模拟编程"时，更好的回应是封装一个专用节点，而不是引入通用脚本。
