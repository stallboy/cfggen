        ...
        Sequence { acts: [
            ...
            WithActorControl {
                ...
                body: Sequence { acts: [
                    ...
                    ApplyEffect { target: ContextTargets{}; effect: GrantTags { grantedTags: ["State.Invincible"]; duration: Const{value: 3.0} } };
                    // 3. 镜头聚焦特写
                    ...
                    ApplyEffect { target: ContextTargets{}; effect: ApplyStatus { statusId: 8002; bindings: [] } } 
                ]}
            }
        ]}
    ]}
        ...
        behaviors: [
            Trigger {
                ...
                effect: Sequence { effects: [
                    ...
                    // 3. 广播一个全局事件给 Scene 系统
                    SendEvent { event: "Event_Global_LoudNoise"; magnitude: Const{value: 1.0}; extras: [] },
                    // 4. 销毁自身
                    ApplyEffect { effect: GrantTags { grantedTags: ["State.PendingDestroy"]; duration: Const{value: 0.1} }; bindings: [] }
                ]};
            }
        ];
    };
...
scene_definition {
    ...
    rootAct: Parallel { acts: [
        ...
        // 全局监听：一旦有人弄出巨大声响，立刻拉响警报，刷出重甲兵
        WaitForEvent {
            eventTag: "Event_Global_LoudNoise";
            source: SceneVar { actorVar: "Sys.Global" }; // 监听全局播报者
            conditions: []; extractPayloads: []; timeoutSec: -1.0; onTimeout: None {};
            
            body: Sequence { acts: [
                PlayCue { cueKey: "Cue.Siren_Alarm"; playAt: ContextTargets{} },
                SpawnActor { outputVarKey: "Var.Guard"; archetype: "Archetype_HeavyGuard"; spawnAt: "SpawnPoint_A" },
                SetSceneVar { bindings: [{ varKey: "IsStealthBroken"; value: Float { value: Const{value: 1.0} } }] }
            ]}
        }
    ]}
}
...
### 案例三：硬控打断与机制反制（施法管线交锋）

**场景描述**：
法师怪物正在读条（CastBar）一个极具毁灭性的陨石法术。玩家抓准时机，使用瞬发的“盾击”技能造成眩晕。由于 TagRules 的介入，怪物的读条被强制硬打断（interruptsAbilities），引发法术反噬，且 AI 大脑收到失败反馈进入下一帧重选。

**数据流向**：
`AI (发起 CastAbility)` ➡️ `GAS (进入 CastBar 读条期)` ➡️ `Player GAS (盾击挂载 Stun 标签)` ➡️ `GAS TagRules (Stun 标签引发 interruptsAbilities 截断施法)` ➡️ `AI (Task 返回 Failed，放弃执行树)`

#### 1. AI 配置（发起意图）
```text
ai_behavior {
    behaviorId: 6010; name: "Behavior_Cast_Meteor";
    requiredGoal: { goalDef: "Goal.Target.Player"; selector: Nearest };
    preConditions: [ { CanActivateAbility { abilityId: 6010 } } ];
    score: Const { value: 60.0 };
    scoreInertia: 0.0;
    
    // 发起施法。底层会阻塞在这里等待生命周期结果（Completed/Cancelled/Interrupted）
    task: CastAbility { abilityId: 6010; target: BoundGoalActor{} };
    abortConditions: [];
}
```
...
// 1. 怪物的陨石技能（带长读条）
ability {
    id: 6010; name: "Ability_Meteor_Strike";
    abilityTags: ["Ability.Type.Spell"];
    targeting: SingleTarget { allowedRelations: [Hostile]; tagQuery: {requireAny:[]}; maxRange: Const{value: 20.0}; onTargetLost: Cancel };
    activationConditions: []; costs: []; cooldown: Const{value: 10.0};
    
    castMode: CastBar {
        castTime: Const { value: 3.0 }; // 读条 3 秒
        castingTags: ["State.Casting.Spell"];
        cuesDuringCast: ["Cue.Cast.Meteor"];
        commitPolicy: OnComplete; 
    };
    
    // 顺利读完条的出伤效果
    effect: ResolveCombat { pipeline: "MagicDamage"; magnitude: Const{value: 500.0}; tags: ["Damage.Element.Fire"]; cuesOnExecute: ["Cue.Impact.Meteor"] };
    
    // 【核心联动】：在 Processing 阶段被硬控打断时执行的惩罚动作
    onInterrupt: [
        ApplyStatus { statusId: 999; bindings: [] } // 挂载“法术反噬虚弱” Status
    ];
    recovery: { duration: Const{value: 0.5}; recoveryTags: ["State.Recovery"]; cuesDuringRecovery: [] };
}

// 2. 玩家的盾击技能（瞬发，挂眩晕）
ability {
    id: 101; name: "Ability_Shield_Bash";
    abilityTags: ["Ability.Type.Melee"];
    targeting: SingleTarget { allowedRelations: [Hostile]; tagQuery: {requireAny:[]}; maxRange: Const{value: 3.0}; onTargetLost: Cancel };
    activationConditions: []; costs: []; cooldown: Const{value: 5.0};
    
    castMode: Instant {}; // 无前摇
    effect: WithTarget {
        // 读取由底层 TargetingRequirement 收集并写入的标准目标变量
        target: ContextVar { varKey: "Sys.Targeting.Actor" }; 
        effect: Sequence { effects: [
            ResolveCombat { pipeline: "PhysicsDamage"; magnitude: Const{value: 50.0}; tags: ["Damage.Type.Blunt"]; cuesOnExecute: [] },
            ApplyStatus { statusId: 120; bindings: [] } // 挂载 Stun Status
        ]};
    };
    onInterrupt: [];
    recovery: { duration: Const{value: 0.3}; recoveryTags: ["State.Recovery"]; cuesDuringRecovery: [] };
}

status {
    id: 120; name: "Status_Stunned";
    stackingPolicy: Single { refreshMode: ResetDuration };
    core: StatusCore {
        grantedTags: ["State.Debuff.Control.Stun"]; // 👈 将引发全局断法的标签
        duration: Const { value: 2.0 };
        cuesWhileActive: ["Cue.Status.Stun"];
        behaviors: [];
    };
}

// 3. 全局战斗法则（TagRules 裁定）
tag_rules {
    name: "Global_Combat_Rules";
    rules: [
        {
            // 物理准则：当任何人获得 Stun 标签时...
            whenPresent: "State.Debuff.Control.Stun";
            // 强行打断他正在施放的所有属于 Spell 分类的技能，并触发 onInterrupt 反噬！
            interruptsAbilities: ["Ability.Type.Spell"]; 
            blocksAbilities: ["Ability.Type.Spell"];
            immuneToTags: [];
            purgesTags: [];
            description: "眩晕：硬打断法术并引发反噬";
        }
    ]
}
```

**💡 架构之美**：这是一个教科书般的**控制反转**。怪物 AI 不用写“如果我被打晕我要取消施法”，玩家的盾击也不用写“我要取消怪物的陨石术”。一切的交互均在 `TagRules` 这个“最高物理法则”中发生碰撞，系统如齿轮般严丝合缝。


### 案例四：护送防守玩法（持续引导 + 硬控打断与反噬）
...
ability {
    ...
    effect: Sequence { effects: [
        ResolveCombat { pipeline: "PhysicsDamage"; magnitude: Const{value: 50.0} };
        // 开放 1.5 秒的二段连招窗口
...
ai_behavior {
    ...
    preConditions: [
        ...
        { TargetHasTags { target: ThinkActor{ BoundGoalActor{} }; tagQuery: { requireAll: ["State.Casting.Heavy"] } } }
    ];
...
### 案例六：动态目标追踪与“断线”机制（Targeting 追踪）
...
ai_behavior {
    ...
    // 感知到血量不满的友军（通常是 Boss）
    requiredGoal: { goalDef: "Goal.Friendly.NeedsHeal"; selector: LowestHP };
    ...
    task: Sequence { tasks: [
        ...
        Parallel {
            ...
            tasks: [
                CastAbility { abilityId: 602; target: BoundGoalActor{} }; // 引擎会自动把目标写入 TargetVar
                Loop { count: -1.0; task: 
                    MoveTo { target: BoundGoalActor{}; tolerance: 10.0 } 
                }
            ]
