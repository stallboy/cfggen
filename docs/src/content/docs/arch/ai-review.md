---
title: AI 系统设计 review
sidebar:
  order: 12
---

> 本文是设计评审（决策备忘 + 行业对照），不是评审基准。权威定义以 [`ai-design.md`](./ai-design.md) 为准。文中业界信息以公开资料为准，不逐条核实。

##  5 个常见思维模式

###  1. 感知-决策-执行的三段式分离（Sense-Think-Act）

这是游戏 AI 架构中常见的共识。从 Brooks 的包容架构到现代商业引擎，感知系统不应该直接触发动作，决策系统不应该自己去采集世界数据，执行系统不应该反过来修改决策逻辑。
...
AI 行为必须由配置数据（表格、脚本资产、JSON）而非编译型代码定义。这一点有较强的实践基础：
...
AI 系统决定"做什么"和"对谁做"，但"怎么做"的物理表现交给下游系统（GAS、动画状态机、物理引擎）。你的文档中表述为"AI 是大脑，GAS 是躯体"，这一原则在业界普遍采用。
...
Bobby Anguelov 在 GDC 演讲中反复强调的核心观点——如果你无法在运行时清晰地看到 AI 为什么选择了这个行为而不是那个，你的系统就难以维护。
...
你的设计文档将"白盒可调试性"定为一级约束，架构上支持 L2（扁平评分天然支持算分明细展示）。若复杂 Boss 战排查频繁，L3（时间轴回溯）值得规划——问题往往出现在“30 秒前的那次重选”，而不是当前帧。当前 L2 已覆盖主要场景。

---

###  5. 打断机制必须显式设计（Interruption is a First-Class Concern）

AI 行为最复杂的不是"开始做什么"，而是"什么时候停下来"。打断不是异常处理，而是正常流程的一部分，必须在架构层面显式支持。
...
**取舍代价**：当前设计对单体 AI 足够。团队协作 AI（如小队战术）需要跨个体共享目标分配，在本架构里需通过外部系统向各 AI 的感知层注入 Goal 来实现（“用 Goal 注入替代黑板写入”），而不是共享黑板——这是分布式上下文路线的固有代价。

---

###  对分歧三（层级式组织 vs. 完全扁平池）的判断

**我的立场：你的设计的折中方案（配置分组 + 运行时扁平）是工程上的务实解，但需要在文档中更诚实地承认 sharedConditions 的隐式层级性质。**
...
**取舍代价**：`sharedConditions` 在运行时确实被评估——某 Group 的 sharedCondition 失败则该组全部行为被跳过，这事实上是运行时的一次短路剪枝。调试时需要把“组级门控”单独显示（一眼看到“不是行为 X 自身条件不满足，而是它所在的近战组被整体跳过了”），否则会误导排查。这是“配置分组 + 运行时扁平”折中的固有代价，spec 当前没有为此提供专门字段。
...
| 分歧 | 你的立场 | 评价 |
|:---|:---|:---|
| 行为树 vs. 效用系统 | 效用决策 + tree执行 | 混合路线 |
| 黑板 vs. 分布式上下文 | Goal 驱动 + 阶段隔离上下文 | 需为协作 AI 预留扩展 |
| 扁平 vs. 层级 | 配置分组 + 运行时扁平 | 务实折中，调试需暴露组级剪枝 |

这三个选择共同构成了一套**立场清晰、可落地**的 AI 行为系统。它不是所有场景的解（高度叙事驱动的线性游戏可能仍偏好行为树的精确编排），但对于动作 RPG、ARPG、开放世界等需要大量 AI 实体表现出自然、多样行为的游戏类型，是一个可用的架构选择。