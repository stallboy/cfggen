...
```bash
Usage: java -jar cfggen.jar [options] -datadir [dir] [options] [gens]
...
-----tools
    ...
    -compareterm      检查翻译名词表
    -usepoi           用poi库去读excel文件，会比较慢，支持数字格式，默认false
    -comparepoiandfastexcel   对默认的fastexcel库读出的excel文件单元格和poi读出来的结果做比较，打印出不同处。
-----options
